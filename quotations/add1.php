...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pol           = trim($_POST['pol'] ?? '');
    $pod           = trim($_POST['pod'] ?? '');
    // FIX: dùng ?? '' thay vì truy cập trực tiếp (PHP 8.4 strict)
    ...
    if (empty($error)) {
        ...
        try {
            $stmt = $conn->prepare(
                "INSERT INTO quotations (quotation_no, customer_id, issue_date, valid_until, currency, exchange_rate, pol, pod, packages, gw, cw, notes, status, created_by)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            );
