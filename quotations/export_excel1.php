...
function vnFull($v, $d = 2): string {
    // Luôn hiển thị đủ số thập phân (dùng cho tiền)
    $v = floatval($v);
...
// Columns: A=pad(1), B=Desc(28), C=Currency(10), D=UnitPrice(14),
//          E=Qty(10), F=Amount(16), G=ExchRate(12), H=VAT(8), I=TotalVND(18), J=Notes(18), K=pad(1)
$cw = ['A'=>1,'B'=>28,'C'=>10,'D'=>14,'E'=>10,'F'=>16,'G'=>12,'H'=>8,'I'=>18,'J'=>18,'K'=>1];
foreach ($cw as $col => $w)
...
$shipInfo = [];
if (!empty($pol)) $shipInfo[] = ['POL (Cảng đi):', $pol];
if (!empty($pod)) $shipInfo[] = ['POD (Cảng đến):', $pod];
if (!empty($packages)) $shipInfo[] = ['Số kiện (Qty):', vn($packages, 0) . ' kiện'];
if (!empty($gw))       $shipInfo[] = ['GW:', vnFull(floatval($gw), 2) . ' KGS'];
if (!empty($cw))       $shipInfo[] = ['CW:', vnFull(floatval($cw), 2) . ' KGS'];
$shipInfo[] = ['Ngày lập (Date):', $quot['issue_date'] ? date('d/m/Y', strtotime($quot['issue_date'])) : ''];
if (!empty($quot['valid_until']))
    $shipInfo[] = ['Hiệu lực đến (Valid until):', date('d/m/Y', strtotime($quot['valid_until']))];

foreach ($shipInfo as [$l, $v]) {
...
// Nhóm items theo tiền tệ để tính tổng
...
foreach ($items as $idx => $item) {
    ...
    // Tỷ giá — để trống vì quotation không có exchange_rate per item
    ...
    // Tổng VND — dùng amount (đây là quotation nên tổng = thành tiền * tỷ giá nếu có)
...
// ══════════════════════════════════════════════════════════════════════
// SECTION 7 — CHỮ KÝ (chỉ người lập, góc phải)
// ══════════════════════════════════════════════════════════════════════
$sheet->getRowDimension($r)->setRowHeight(14);
...
// 3 dòng trống để ký tên
for ($i = 0; $i < 3; $i++) {
    $sheet->getRowDimension($r)->setRowHeight(16);
    $r++;
}

// Tên công ty dưới chữ ký — căn giữa, KHÔNG có border
$sheet->getRowDimension($r)->setRowHeight(14);
$sheet->mergeCells("G{$r}:J{$r}");
