...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // FIX: dùng ?? '' để tránh lỗi PHP 8.4 undefined array key
    if ($hasNewCols) {
        $pol      = trim($_POST['pol'] ?? '');
        $pod      = trim($_POST['pod'] ?? '');
        $pkg_raw  = $_POST['packages'] ?? '';
        $gw_raw   = $_POST['gw']       ?? '';
        $cw_raw   = $_POST['cw']       ?? '';
        $packages = ($pkg_raw !== '') ? floatval($pkg_raw) : null;
        $gw       = ($gw_raw  !== '') ? floatval($gw_raw)  : null;
        $cw       = ($cw_raw  !== '') ? floatval($cw_raw)  : null;
    }
            ...
            if ($hasNewCols) {
                $stmt = $conn->prepare(
                    "UPDATE quotations SET quotation_no=?, customer_id=?, issue_date=?, valid_until=?,
                     currency=?, exchange_rate=?, pol=?, pod=?, packages=?, gw=?, cw=?, notes=?, status=?
                     WHERE id=?"
                );
                $stmt->bind_param("sisssdssdddssi",
                    $quotation_no, $customer_id, $issue_date, $valid_until,
                    $currency, $exchange_rate,
                    $pol, $pod, $packages, $gw, $cw,
                    $notes, $status, $id
                );
            } else {
                $stmt = $conn->prepare(
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        <div class="alert alert-warning py-2 mb-0">
                            ...
                            Chưa có cột <strong>POL / POD / Số kiện / GW / CW</strong> trong database.
                            Vui lòng chạy file <code>alter_quotations.sql</code> trong phpMyAdmin để kích hoạt các trường này.
                        </div>
                    </div>
                    <?php endif; ?>
