...
class EHoaDonClient
{
    ...
    public function buildInvoicePayload(
        ...
        // ── Dòng GhiChu đầu hóa đơn: chỉ hiện số tờ khai HOẶC số vận đơn ──
        // Ưu tiên số tờ khai; nếu không có thì dùng số vận đơn
        if ($tkHQ !== '') {
        ...
        // ── Note (trường ghi chú nội bộ): Job No + tờ khai hoặc vận đơn + extraNote ──
        // Ưu tiên số tờ khai; nếu không có thì dùng số vận đơn
        $note = $shipment['job_no'];
        if ($tkHQ !== '') {
            $note .= ' | ' . $tkHQ;
        } elseif ($hawb !== '') {
            $note .= ' | ' . $hawb;
