...
$view        = trim($_GET['view']        ?? 'shipment'); // 'shipment' | 'customer'
...
$sql = "SELECT s.id, s.job_no, s.hawb, s.mawb, s.customs_declaration_no,
    ...
    c.id AS cust_id, c.company_name, c.short_name,
...
if ($export_soa && $soa_customer > 0) {
    ...
    if ($soa_data) {
        ...
        foreach ($soa_data['shipments'] as $r) {
            $remain = $r['kh_remain'];
            $status_txt = $remain <= 0 ? '<span class="paid">Đã thanh toán</span>' : ($r['customer_paid_amount'] > 0 ? '<span style="color:orange;">Một phần</span>' : '<span class="unpaid">Chưa thanh toán</span>');
            echo '<tr>
        ...
        <div class="d-flex gap-2">
            ...
            <div class="btn-group btn-group-sm">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['view'=>'shipment'])); ?>"
                   class="btn <?php echo $view==='shipment' ? 'btn-primary' : 'btn-outline-primary'; ?>">
                    <i class="bi bi-list-ul"></i> Theo Lô
                </a>
                <a href="?<?php echo http_build_query(array_merge($_GET, ['view'=>'customer'])); ?>"
                   class="btn <?php echo $view==='customer' ? 'btn-primary' : 'btn-outline-primary'; ?>">
...
// Thêm đoạn này vào cuối file, trước thẻ </body>
document.addEventListener('DOMContentLoaded', function () {
    const form = document.querySelector('.card-body form');

    // Các select tự động submit khi thay đổi
    const autoSubmitSelects = form.querySelectorAll('select');
    ...
    // Input month tự động submit khi thay đổi
    ...
    // Ô search: submit sau khi dừng gõ 500ms (debounce)
