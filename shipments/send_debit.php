...
$auto_subject = 'DEBIT // DFAST // ' . $hawb . (!empty($cd_no) ? ' // ' . $cd_no : '');
...
function buildExcelFile($shipment, $sells) {
    ...
    $merge("D{$r}:H{$r}");
    $setCell("D{$r}", 'DFAST LOGISTICS CO.,LTD');
    ...
    $merge("D{$r}:H{$r}"); $setCell("D{$r}", 'No. 58 Lien Xa Street, Thien Loc Commune, Hanoi City, Vietnam');
    $sStyle("D{$r}", ['font' => ['size' => 9, 'color' => ['rgb' => '444444'], 'name' => 'Calibri'], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER]]); $rowH($r, 14); $r++;

    $merge("D{$r}:H{$r}"); $setCell("D{$r}", 'Tel: (+84) 366 666 322     Email: dungdaongocanh@gmail.com');
    $sStyle("D{$r}", ['font' => ['size' => 9, 'color' => ['rgb' => '444444'], 'name' => 'Calibri'], 'alignment' => ['horizontal' => Alignment::HORIZONTAL_CENTER]]); $rowH($r, 14); $r++;

    $merge("D{$r}:H{$r}"); $setCell("D{$r}", 'MST / Tax Code: 0111372067');
    ...
    $bankRows = [
        ['Chu TK:', 'CONG TY TNHH DFAST LOGISTICS', 'VU THUY LINH'],
        ['So TK:', '67339999', '19032342305016'],
        ['Ngan hang:', 'Techcombank', 'Techcombank'],
        ['Chi nhanh:', 'Ha Noi', 'Ha Noi'],
        ['Noi dung CK:', 'DFAST - ' . ($shipment['job_no'] ?? '') . ' - ' . ($shipment['hawb'] ?? ''), 'POB - ' . ($shipment['job_no'] ?? '')],
    ];
    ...
    $rowH($r, 16); $merge("B{$r}:D{$r}"); $setCell("B{$r}", 'DFAST LOGISTICS CO.,LTD');
