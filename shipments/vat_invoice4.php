...
// ── Use LEFT JOIN + COALESCE for description ───────────────────────────────
...
// ── Ghi chú mặc định: ưu tiên số tờ khai → số vận đơn ───────────────────
$customs     = trim($shipment['customs_declaration_no'] ?? '');
$hawb        = trim($shipment['hawb'] ?? '');
$defaultNote = !empty($customs) ? $customs : (!empty($hawb) ? $hawb : '');
// ─────────────────────────────────────────────────────────────────────────
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'issue') {
        if (empty($shipment['customer_tax'])) {
            ...
            // Nếu người dùng không nhập → dùng số tờ khai hoặc vận đơn
            if ($extraNote === '') {
                $extraNote = $defaultNote;
            }

            // ── Chọn nhóm hàng theo nút bấm ─────────────────────────────
            ...
            if (empty($sellsToIssue)) {
                ...
                try {
                    ...
                    if ($client->isSuccess($result)) {
                        ...
                        // ── Cập nhật $shipment trong memory đúng cột ─────
                        if ($invoiceGroup === 'vat0') {
                            ...
                            <form method="POST" id="issueForm">
                                ...
                                <div class="row g-3 mb-3">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <input type="text" name="extra_note" class="form-control"
                                               placeholder="<?php echo htmlspecialchars($defaultNote ?: 'VD: Tờ khai số 123456...'); ?>">
                                        <div class="form-text">
                                            <?php if (!empty($customs)): ?>
                                                Tự động điền: <code><?php echo htmlspecialchars($customs); ?></code>
                                                <span class="text-muted">(Số tờ khai)</span>
                                            <?php elseif (!empty($hawb)): ?>
                                                Tự động điền: <code><?php echo htmlspecialchars($hawb); ?></code>
                                                <span class="text-muted">(Số vận đơn — không có số tờ khai)</span>
                                            <?php else: ?>
                                                <span class="text-muted">Không có số tờ khai hoặc số vận đơn.</span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
