...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save') {
    ...
    if (empty($errors)) {
        ...
        // Auto-sync SELL lines
        $pol          = $shipment['pol']           ?? '';
        $pod          = $shipment['pod']           ?? '';
        $vesselFlight = $shipment['vessel_flight'] ?? '';

        $fetchCC = $conn->prepare("SELECT id,code FROM cost_codes WHERE code IN ('FREIGHT','LOCAL-SERVICE') LIMIT 2");
        $fetchCC->execute();
        $ccRows = $fetchCC->get_result()->fetch_all(MYSQLI_ASSOC);
        $fetchCC->close();
        $ccMap = [];
        foreach ($ccRows as $r) $ccMap[$r['code']] = (int)$r['id'];

        $exwCheck = $conn->prepare(
            "SELECT COUNT(*) AS cnt FROM arrival_notice_charges
             WHERE shipment_id=? AND charge_group='foreign' AND UPPER(cost_code)='EXW'"
        );
        $exwCheck->bind_param('i', $id); $exwCheck->execute();
        $hasExw = ($exwCheck->get_result()->fetch_assoc()['cnt'] ?? 0) > 0;
        $exwCheck->close();

        $freightDesc = "Cước vận chuyển quốc tế ({$pol} - {$pod} // Chuyến Tàu/bay: {$vesselFlight})";
        if ($hasExw) $freightDesc .= " và Phí tại đầu ({$pol})";

        $syncSell = function(int $ccId, string $desc, float $amount) use ($conn, $id) {
            $chk = $conn->prepare("SELECT id FROM shipment_sells WHERE shipment_id=? AND cost_code_id=? LIMIT 1");
            $chk->bind_param('ii', $id, $ccId); $chk->execute();
            $existing = $chk->get_result()->fetch_assoc(); $chk->close();
            if ($existing) {
                $u = $conn->prepare("UPDATE shipment_sells SET description=?,total_amount=? WHERE id=?");
                $u->bind_param('sdi', $desc, $amount, $existing['id']); $u->execute(); $u->close();
            } elseif ($amount > 0) {
                $ins2 = $conn->prepare("INSERT INTO shipment_sells (shipment_id,cost_code_id,description,total_amount) VALUES (?,?,?,?)");
                $ins2->bind_param('iisd', $id, $ccId, $desc, $amount); $ins2->execute(); $ins2->close();
            }
        };

        if (!empty($ccMap['FREIGHT']))       $syncSell($ccMap['FREIGHT'],       $freightDesc,             $foreignTotalVnd);
        if (!empty($ccMap['LOCAL-SERVICE'])) $syncSell($ccMap['LOCAL-SERVICE'], 'Phí dịch vụ tại Việt Nam', $localTotalVnd);
